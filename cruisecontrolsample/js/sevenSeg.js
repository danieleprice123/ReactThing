...
displayValue: function(value) {
	...
	//start: convert e annotation
	var fixedDigits = undefined;
	if (sValue.indexOf('e-') != -1) {
		fixedDigits = parseInt(sValue.split('e-')[1]);
	} else if (sValue.indexOf('E-') != -1) {
		fixedDigits = parseInt(sValue.split('E-')[1]);
	}
	
	if (fixedDigits !== undefined) {
		sValue = value.toFixed(fixedDigits).toString();
	}
	//end: convert e annotation
	
    var iDecimalIdx = sValue.indexOf('.');
    var iDigitIdx = sValue.length - 1;
	
    //start: align from left to right
	var valueLength = iDecimalIdx > -1 ? sValue.length - 1 : sValue.length;
	if (iDecimalIdx > -1 && this.options.digits < valueLength) {
		iDigitIdx = this.options.digits ;
	}
	//end: align from left to right
